...
//#define SETUP_ID            "VERSION1|ITTF000"			//Generic setup id
...
void write_data_flash_block( Uint32 dest_ptr, Uint32 src_ptr, Uint16 byte_count);
...
void poll_adc(void)
{
	adc_values.v_bus_sense = AdcRegs.ADCRESULT[(0)].bit.RESULT;
//	adc_values.i_bus_sense = AdcRegs.ADCRESULT[IBUS].bit.RESULT;
	adc_values.v_bat_sense = AdcRegs.ADCRESULT[(1)].bit.RESULT;
	adc_values.i_bat_sense = AdcRegs.ADCRESULT[(3)].bit.RESULT;

	adc_values_avg.v_bus_sense = AdcRegs.ADCAVGRESULT[(0)].bit.RESULT;
//	adc_values_avg.i_bus_sense = AdcRegs.ADCAVGRESULT[IBUS].bit.RESULT;
	adc_values_avg.v_bat_sense = AdcRegs.ADCAVGRESULT[(1)].bit.RESULT;
	adc_values_avg.i_bat_sense = AdcRegs.ADCAVGRESULT[(3)].bit.RESULT;
...
{
	if((adc_values_avg . v_bus_sense < ((35 * ((Uint32)((0.026 * 10) * 4096 / 2.5))) / 10)))//if Vbus too low
	{
	...
	else
	{
		...
		fault_type = NO_FAULT;
		charge_state = (0);
		buck_hysteresis = ((Uint32)(((4) * ((Uint32)((0.026 * 10) * 4096 / 2.5))) / 10));
		boost_hysteresis = ((Uint32)(((4) * ((Uint32)((0.026 * 10) * 4096 / 2.5))) / 10));
		...
		MiscAnalogRegs.GLBIOVAL.all = 0;
		MiscAnalogRegs.GLBIOEN.all = (1 << 26) | (1 << 24) | (1 << 25) | (1 << 11) | (1) | (1 << 1) | (1 << 2) | (1 << 3);
		...
		FeCtrl1Regs.RAMPDACEND.bit.RAMP_DAC_VALUE = (2) * ((Uint32)((0.15556) * 16384 / 1.6));
		...
		if((adc_values_avg . v_bat_sense < ((53 * ((Uint32)((0.026 * 10) * 4096 / 2.5))) / 10)))
		{
		...
		{
			MiscAnalogRegs.GLBIOVAL.all = 0;
			MiscAnalogRegs.GLBIOEN.all = (1) | (1 << 1);
...
inline void ramp_up_state_handler(void)
{
	...
	if(adc_values.v_bus_sense > (adc_values.v_bat_sense + buck_hysteresis))//buck
	{
		if(charge_state != (1))
		{
			...
			if(supply_state != STATE_BACKUP)//fast interrupt may change the state to back up
			{
				...
				MiscAnalogRegs.GLBIOEN.all = (1 << 26) | (1 << 24) | (1 << 25) | (1 << 11) | (1 << 2) | (1 << 3);//Q3 off to prevent reverse current
				charge_state = (1);
			}
			...
			buck_hysteresis = ((Uint32)(((4) * ((Uint32)((0.026 * 10) * 4096 / 2.5))) / 10));
			boost_hysteresis = ((Uint32)(((4) * ((Uint32)((0.026 * 10) * 4096 / 2.5))) / 10)) + ((Uint32)(((1.5) * ((Uint32)((0.026 * 10) * 4096 / 2.5))) / 10));
		}
	}
	else if(adc_values.v_bus_sense > (adc_values.v_bat_sense - boost_hysteresis))//buck-boost
	{
		if(charge_state != (2))
		{
			...
			if(supply_state != STATE_BACKUP)
			{
				...
				MiscAnalogRegs.GLBIOEN.all = (1 << 26) | (1 << 24) | (1 << 25) | (1 << 11);
				charge_state = (2);
			}
			FaultMuxRegs.DCOMPCTRL0.bit.INT_EN = 1; //enable BUS UVP

			buck_hysteresis = ((Uint32)(((4) * ((Uint32)((0.026 * 10) * 4096 / 2.5))) / 10)) + ((Uint32)(((1.5) * ((Uint32)((0.026 * 10) * 4096 / 2.5))) / 10));
			boost_hysteresis = ((Uint32)(((4) * ((Uint32)((0.026 * 10) * 4096 / 2.5))) / 10)) + ((Uint32)(((1.5) * ((Uint32)((0.026 * 10) * 4096 / 2.5))) / 10));
		}
	}
	else //boost
	{
		if(charge_state != (3))
		{
			...
			if(supply_state != STATE_BACKUP)
			{
				...
				MiscAnalogRegs.GLBIOVAL.all = (1 << 1);	//turn on Q2
				for(i=0;i<40;i++)
				{
					asm(" nop");
				}
				MiscAnalogRegs.GLBIOVAL.all = 0;	//turn off Q2
				MiscAnalogRegs.GLBIOVAL.all = (1);	//turn on Q1
				MiscAnalogRegs.GLBIOEN.all = (1 << 26) | (1 << 24) | (1 << 25) | (1 << 11) | (1) | (1 << 1);
				charge_state = (3);
			}
			FaultMuxRegs.DCOMPCTRL0.bit.INT_EN = 1; //enable BUS UVP
	...
	{
		FeCtrl1Regs.EADCDAC.all = (2) * ((Uint32)((0.15556) * 16384 / 1.6));
...
inline void cc_charge_state_handler(void)
{
	...
	if((adc_values_avg . v_bat_sense > (((57) * ((Uint32)((0.026 * 10) * 4096 / 2.5))) / 10)))
	{
		FaultMuxRegs.DCOMPCTRL0.bit.INT_EN = 0; //disable BUS UVP
	...
	else if((adc_values . i_bat_sense > ((7) * ((Uint32)((0.24) * 4096 / 2.5)))))
	{
		FaultMuxRegs.DCOMPCTRL0.bit.INT_EN = 0; //disable BUS UVP
		...
		if(adc_values.v_bus_sense > (adc_values.v_bat_sense + buck_hysteresis))//buck
		{
			if(charge_state != (1))
			{
				...
				if(supply_state != STATE_BACKUP)//fast interrupt may change the state to back up
				{
					...
					MiscAnalogRegs.GLBIOEN.all = (1 << 26) | (1 << 24) | (1 << 25) | (1 << 11) | (1 << 2) | (1 << 3);
					charge_state = (1);
				}
				FaultMuxRegs.DCOMPCTRL0.bit.INT_EN = 1; //enable BUS UVP

				MiscAnalogRegs.GLBIOVAL.all = (1 << 2);	//turn on Q3
			...
			if(interrupt_counter1 > 19)
			{
				...
				asm(" nop");//about 120ns
				MiscAnalogRegs.GLBIOVAL.all = (1 << 3);	//turn on Q4
				asm(" nop");
...
inline void cv_charge_state_handler(void)
{
	...
	if((adc_values_avg.i_bat_sense < ((((2) * ((Uint32)((0.24) * 4096 / 2.5)) * 3) / 100) + ibat_adc_offset)) || ((adc_values_avg . v_bat_sense > (((57) * ((Uint32)((0.026 * 10) * 4096 / 2.5))) / 10))))
	{
		FaultMuxRegs.DCOMPCTRL0.bit.INT_EN = 0; //disable BUS UVP
...
{
	if((adc_values_avg . v_bus_sense > (vbus_norminal + (((5) * ((Uint32)((0.026 * 10) * 4096 / 2.5))) / 10))))
	{
		shut_down();
		fault_type = BUS_OVP;
		interrupt_counter1 = 0;
	...
	else if((adc_values_avg . v_bat_sense < ((38 * ((Uint32)((0.026 * 10) * 4096 / 2.5))) / 10)))
	{
		shut_down();
		fault_type = BAT_UVP;
		interrupt_counter1 = 0;
...
//			if(adc_values.v_bus_sense > vbus_backup_voltage)//VBUS is back
			...
			{
				MiscAnalogRegs.GLBIOEN.all |= (1 << 4);
				MiscAnalogRegs.GLBIOVAL.all |= (1 << 4);
				interrupt_counter2 ++;
