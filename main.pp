...
typedef enum
{
 ...
 Uint32 interrupt_counter1, interrupt_counter2, interrupt_counter3;
 Uint32 hiccup_counter_oc;
 Uint32 hiccup_counter_ov;

 int32 current_share_kp;
 int32 current_error;
...
////////////////////////////////////////////////////////// 
 ...
 Uint32 debug1,debug2;

 Uint32 charge_state;
 Uint32 buck_hysteresis, boost_hysteresis;
 Uint32 pulse_state;
 Uint32 backup_duty;
 Uint32 vbus_filtered, vbus_backup_voltage, vbus_norminal;
 Uint32 buck_duty, buckboost_duty, boost_duty;
 Uint32 ibat_adc_offset;
...
//		pmbus_write_rom_mode();
	...
	while((adc_values_avg.v_bat_sense < 800) || (adc_values_avg . v_bus_sense < ((35 * ((Uint32)((0.026 * 10) * 4096 / 2.5))) / 10)))//no battery or bus too low
	{
		pmbus_handler();
		poll_adc();
	}
	...
	for(;;)
	{
		...
		if((supply_state != STATE_BACKUP))// && (supply_state != STATE_FAULT))
		{
			...
			FeCtrl2Regs.EADCDAC.all = ((vbus_norminal * 100) >> 4) - (((0.5) * ((Uint32)((0.026 * 10) * 16384 / 1.6))) / 10);
			vbus_backup_voltage = vbus_norminal - (((0.5) * ((Uint32)((0.026 * 10) * 4096 / 2.5))) / 20);
