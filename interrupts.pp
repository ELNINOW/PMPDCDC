...
//	if(temp & 0x2000)
	{
		...
		LoopMuxRegs.DPWMMUX.bit.DPWM0_FILTER_SEL = 2; //CLA2 is providing input to DPWM0
		LoopMuxRegs.DPWMMUX.bit.DPWM1_FILTER_SEL = 2; //CLA2 is providing input to DPWM1

		MiscAnalogRegs.GLBIOEN.all = (1 << 26) | (1 << 24) | (1 << 25) | (1 << 11);// | MASK_DPWM0A;// | MASK_DPWM1B;
		Dpwm0Regs.DPWMCTRL0.all |= 0x33000000; //swap 0A->0B, 0B->0A
		Dpwm1Regs.DPWMCTRL0.all &= ~0xFF000000; //1A->1A, 1B->1B
